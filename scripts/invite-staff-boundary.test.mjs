  ...
  function read(){
   ...
   if(table==='staff'){
    ...
    if(filters.email!=null){
     const matched=typeof filters.email==='string' && filters.email.replace(/\\([\\%_])/g,'$1').toLowerCase()===storedEmail.toLowerCase()
...
for(const email of ['with_under@example.invalid','with%percent@example.invalid'])test(`メールの記号をワイルドカードとして扱わない: ${email}`,async()=>{
 const h=setup({inviteEmail:email,storedEmail:email,emailOrg:'other'})
 assert.equal((await h.call()).status,403)
 assert.equal(h.writes.length,0)
 assert.deepEqual(h.emailFilters,[email.replace(/[\\%_]/g,'\\$&')])
})
