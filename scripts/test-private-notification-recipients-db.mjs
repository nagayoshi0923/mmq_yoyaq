...
async function request(inputScenario=scenario, dates=['2026-10-19']) {
 ...
 const {rows}=await db.query("SELECT create_private_booking_request($1,$2,'Fixture','fixture@example.invalid','00000000000',6,$3,NULL,NULL,$4) AS id",[inputScenario,customer,JSON.stringify(payload),group])
 return rows[0].id
}
...
const legacyScenario='20000000-0000-0000-0000-000000000099'
const otherOrg='10000000-0000-0000-0000-000000000002'
await db.query('UPDATE organization_scenarios SET id=$1',[legacyScenario])
const staffIds = Array.from({length:6},(_,i)=>`60000000-0000-0000-0000-${String(i+10).padStart(12,'0')}`)
for (let i=0;i<staffIds.length;i++) {
 await db.query("INSERT INTO staff(id,organization_id,name,status) VALUES($1,$2,'fixture',$3)",[staffIds[i],i===3?otherOrg:org,i===2?'resigned':'active'])
 await db.query('INSERT INTO staff_scenario_assignments(staff_id,scenario_id,scenario_master_id,organization_id,can_main_gm,can_sub_gm) VALUES($1,$2,$3,$4,$5,$6)',[staffIds[i],legacyScenario,scenario,i===4?otherOrg:org,i===0 || i===2 || i===3 || i===4,i===1])
}
for (const input of [legacyScenario,scenario]) {
 const booking=await request(input)
 const row=await read(booking)
 assert.equal(row.scenario_id,scenario)
 assert.equal(row.scenario_master_id,scenario)
 assert.equal(row.organization_id,org)
 const pending=(await db.query('SELECT staff_id FROM gm_availability_responses WHERE reservation_id=$1 ORDER BY staff_id',[booking])).rows.map(row=>row.staff_id)
 assert.deepEqual(pending,staffIds.slice(0,2),'only active main/sub GM in the same organization')
}
...
await db.close()
console.log('PASS private notification RPC: legacy/master input, canonical IDs, active main/sub assignment, cross-org/retired/no-role exclusion, rollback/reapply')
