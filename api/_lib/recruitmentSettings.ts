...
export async function recruitmentSettings(req: VercelRequest, res: VercelResponse, user: AuthUser, save = false) {
  ...
  if (save) {
    ...
    if (typeof enabled !== 'boolean' || !['common', 'custom'].includes(source) || !isRecruitmentTarget(mode, value)
    ...
    const { data, error } = await db!.rpc('save_scenario_recruitment_settings_v2', {
      p_organization_id: user.orgId, p_master_id: masterId, p_actor_id: user.userId,
      p_enabled: enabled, p_source: source, p_mode: mode, p_value: value, p_deadline_minutes: deadline_minutes, p_expected_updated_at: expected_updated_at,
    })
  ...
  const { data: setting, error } = await db!.from('organization_scenarios')
    .select('id,recruitment_extension_enabled,recruitment_max_missing,recruitment_deadline_minutes,recruitment_target_source,recruitment_target_mode,recruitment_target_value,override_player_count_min,override_player_count_max,updated_at')
  ...
  const [{ data: common, error: commonError }, { data: master, error: masterError }] = await Promise.all([
    db!.from('organization_recruitment_settings').select('mode,value,updated_at').eq('organization_id', user.orgId).maybeSingle(),
  ...
  const commonTarget = common ?? { ...DEFAULT_RECRUITMENT_TARGET, updated_at: null }
  ...
  return res.status(200).json({ setting, common: commonTarget, min_required: minimum, effective_max_missing: recruitmentMissingLimit(minimum, target), history: (history ?? []).map(h => ({ ...h, actor_name: actors?.find(a => a.user_id === h.actor_id)?.name ?? h.actor_id })), can_edit: ['admin', 'license_admin'].includes(user.role) })
}
...
export async function commonRecruitmentSettings(req: VercelRequest, res: VercelResponse, user: AuthUser, save = false) {
  ...
  if (save) {
    requireAdmin(user)
    const { mode, value, expected_updated_at } = req.body ?? {}
    if (!isRecruitmentTarget(mode, value) || (expected_updated_at !== null && (typeof expected_updated_at !== 'string' || !Number.isFinite(Date.parse(expected_updated_at))))) {
      return res.status(400).json({ error: '対象を1〜20人または1〜100％で指定してください' })
    }
    const { data, error } = await db!.rpc('save_organization_recruitment_settings', {
  ...
  return res.status(200).json({ setting: setting ?? { ...DEFAULT_RECRUITMENT_TARGET, updated_at: null }, common_count: commonCount, custom_count: customCount, can_edit: ['admin', 'license_admin'].includes(user.role) })
}
