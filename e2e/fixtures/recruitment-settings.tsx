...
let common = { mode: 'percent', value: 50, updated_at: null as string | null }
let setting = { recruitment_extension_enabled: true, recruitment_max_missing: 2, recruitment_deadline_minutes: 90, recruitment_target_source: 'common', recruitment_target_mode: 'count', recruitment_target_value: 2, updated_at: '2026-09-08T00:00:00Z' }
...
apiClient.patch = async (path, body) => {
 const value = body as { enabled: boolean; source: string; mode: string; value: number; deadline_minutes: number }
 if (path.includes('common-recruitment-settings')) common = { mode: value.mode, value: value.value, updated_at: new Date().toISOString() }
 else setting = { ...setting, recruitment_extension_enabled: value.enabled, recruitment_target_source: value.source, recruitment_target_mode: value.mode, recruitment_target_value: value.value, recruitment_deadline_minutes: value.deadline_minutes }
