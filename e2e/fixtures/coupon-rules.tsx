...
apiClient.get = async path => {
  ...
  if (path.includes('type=all')) return [{id:'coupon',organization_id:'org',campaign_id:'campaign',customer_id:'customer',uses_remaining:1,status:'active',expires_at:new URLSearchParams(location.search).has('expired')?'2000-01-01T00:00:00Z':'2099-01-01T14:59:59Z',created_at:'2026-09-01',updated_at:new Date().toISOString(),coupon_campaigns:{id:'campaign',name:'25%クーポン',combinable:true,same_scenario_once:true,discount_type:'percentage',discount_amount:25,max_uses_per_customer:1}}] as never
