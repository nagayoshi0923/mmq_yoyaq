...
-- 2. 既存データを新形式に移行（「通常」をデフォルト時間帯として使用）
-- gm_feeが設定されているシナリオを新形式に変換
UPDATE scenarios 
...
SELECT 
  ...
  participation_costs
FROM scenarios 
WHERE jsonb_array_length(gm_costs) > 0 OR jsonb_array_length(license_costs) > 0 OR jsonb_array_length(participation_costs) > 0
LIMIT 5;
*/
