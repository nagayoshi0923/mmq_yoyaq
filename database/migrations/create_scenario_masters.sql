...
-- インデックス
CREATE INDEX IF NOT EXISTS idx_scenario_masters_status ON public.scenario_masters(status);
...
COMMENT ON COLUMN public.scenario_masters.status IS 'draft: 下書き, pending: 承認待ち, approved: 承認済み, rejected: 却下';
...
CREATE TABLE IF NOT EXISTS public.organization_scenarios (
  ...
  status TEXT NOT NULL DEFAULT 'available'
    CHECK (status IN ('available', 'unavailable', 'coming_soon')),
...
CREATE INDEX IF NOT EXISTS idx_org_scenarios_status ON public.organization_scenarios(status);
...
COMMENT ON COLUMN public.organization_scenarios.status IS 'available: 公開中, unavailable: 非公開, coming_soon: 近日公開';
...
CREATE POLICY "scenario_masters_select" ON public.scenario_masters
  ...
  USING (
    ...
    OR auth.uid() IN (SELECT id FROM public.users WHERE role = 'license_admin')
  );
...
CREATE POLICY "scenario_masters_delete" ON public.scenario_masters
  FOR DELETE
  USING (
    auth.uid() IN (SELECT id FROM public.users WHERE role = 'license_admin')
  );
