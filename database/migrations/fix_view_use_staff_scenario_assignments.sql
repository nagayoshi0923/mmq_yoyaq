...
-- 作成日: 2026-02-18
-- 概要: available_gms と experienced_staff を organization_scenarios テーブルから直接取得するのではなく、
--       staff_scenario_assignments テーブルから動的に取得するように変更
--       これにより、担当GM設定が正しく反映される
...
SELECT
  ...
  -- GM関連（staff_scenario_assignments から動的に取得）
  -- 担当GM: can_main_gm または can_sub_gm が true のスタッフ名を配列で取得
  COALESCE(
  ...
  -- 体験済みスタッフ: is_experienced が true かつ担当GMではないスタッフ名を配列で取得
  COALESCE(
    (SELECT ARRAY_AGG(st.name ORDER BY st.name)
     FROM public.staff_scenario_assignments ssa
     JOIN public.staff st ON st.id = ssa.staff_id
...
COMMENT ON VIEW public.organization_scenarios_with_master IS '組織シナリオとマスタを結合したビュー。組織固有設定があればそちらを優先。GM情報はstaff_scenario_assignmentsから動的に取得';
