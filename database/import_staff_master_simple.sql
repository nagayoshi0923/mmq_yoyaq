-- スタッフマスターデータのインポート（シンプル版）
-- ON CONFLICTなし - 初回インポート用
-- 
-- 注意: このSQLは初回インポート専用です。
-- 既存データがある場合は、重複エラーが発生します。
-- 更新する場合は import_staff_master.sql を使用してください（事前にadd_staff_name_unique.sqlが必要）

-- 既存データをクリアする場合はコメントアウトを外してください
-- DELETE FROM staff;
...
-- インポート完了メッセージ
SELECT 
  '✅ スタッフデータのインポートが完了しました（シンプル版）' as status,
