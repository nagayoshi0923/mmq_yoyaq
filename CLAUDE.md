...
**POはこのMacでDiscordを見ている。`http://localhost:<port>` でそのまま開ける。**
**インターネットへの一時公開（トンネル）を提案しない。**

| 見せたいもの | URL |
|---|---|
| yoyaq の開発サーバー | `http://localhost:5174` |
| MMQ-STUDIO の開発サーバー | `http://localhost:5173` |
| 進行モニタ | `http://localhost:5199` |

- **「開けない」と判断する前に、まずサーバーが起動しているかを確認する**。
  落ちているだけなのに「localhostでは見られない」と誤診しない（2026-08-03 実害）。
  yoyaq が落ちていたら `PORT=5174 VITE_DEV_HOST=all npm run dev` で起動する
- POが**スマホや別PCで見たい**と言ったときだけ、同じWi-Fi用に `http://192.168.3.67:<port>` を案内する
  （IPは変わることがあるので `ipconfig getifaddr en0` で確認してから）

### Codexレーンの起動方法（全プロジェクト共通・必守）🚨

Codexへの発注は**必ず**この包み経由で行う。**`codex exec` を直接叩かない。**

```bash
MMQ_EFFORT=high bash ~/.mmq/bin/dispatch-lane.sh <作業名> <発注書の絶対パス> <作業ディレクトリ>
```

- 発注書は**そのセッションの scratchpad ディレクトリ**に置く（パスに含まれるセッションIDから発注元の席が自動記録される）
- 素で `codex exec` を叩くと3つ壊れる: ①stdinが開いたままで固まる ②**進行モニタに一切出ない**（生死・成否を実測できない） ③発注元が記録されず「発注元不明」になる
- 切り離し起動なので即座に返る。完了判定は `~/.mmq/codex-runs/<作業名>.exit`（0なら成功）、ログは同名 `.log`
- 既定モデルは `gpt-5.6-luna`。POがSolを明示した場合、または曖昧・難解・高価値でLunaでは品質不足になる具体的理由がある場合だけ `MMQ_LANE_MODEL=gpt-5.6-sol` を前置きする
- **完了すると常駐ブリッジが、結果と次の実装提案を発注元のPOメッセージへの返信として1通だけ投稿する**。席が完了を待つ必要はなく、依頼書にDiscord投稿コマンドを書かせない
