...
### 発注元への完了報告（2026-08-03 PO指示）

作業が完了したら、**発注元の壁打ちチャンネル（Discord #クインズワルツ👑）へ必ず報告する**。
このリポジトリ側のチャンネルにだけ報告して終えると、発注したPOに結果が届かない。
...
完了時だけでなく、**着手できない・保留にした場合も**同じチャンネルへ理由を投稿する。報告なしでタスクを終えない。
...
ローカルの開発サーバーとモニタは**同じWi-Fi内に公開済み**。**インターネットへの一時公開（トンネル）を提案しない。**

| 見せたいもの | URL |
|---|---|
| yoyaq の開発サーバー | `http://192.168.3.67:5174` |
| MMQ-STUDIO の開発サーバー | `http://192.168.3.67:5173` |
| 進行モニタ | `http://192.168.3.67:5199` |

- **`localhost` のURLを案内しない**（POの端末からは開けない）
- yoyaq の開発サーバーが落ちていたら `PORT=5174 VITE_DEV_HOST=all npm run dev` で起動する
- IPは変わることがある。疑わしいときは `ipconfig getifaddr en0` で確認してから案内する
...
- **完了すると常駐ブリッジが自動でDiscordの発注元チャンネルへ報告を投稿する**（2026-08-03〜）。席が完了を待つ必要はなく、依頼書に報告文を書かせる必要もない
