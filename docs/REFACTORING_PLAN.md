...
- [ ] F-1 削除フローの統合（仕様確定済み）: 有効予約のある公演の削除時、
      「警告ダイアログ → キャンセルメール送信の確認 → OKでメール送信＋予約キャンセル＋公演中止＋削除（履歴に残す）」
      の1フローに統合する。対象は右クリック削除（現状: 拒否のみ）と
      **公演編集モーダルの「この予定を削除」（現状: 予約があってもメール無しで即削除＝危険）**の両方。
...
- [ ] F-3 useEventDelete 内の貸切削除ロジック重複（約80行×2）の統合（分割時に温存したもの）
