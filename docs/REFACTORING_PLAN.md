...
- [x] F-1 削除フローの統合（**完了** 2026-06-13）: 有効予約のある公演の削除時、
      ...
      UI は専用モーダル DeleteEventCancelDialog 1画面（件数＋予約者一覧・理由編集・
      メール送信選択・実行内容まとめ。予約一覧の「予約をキャンセル」と作法を統一。
      当初2ステップ→1画面に収まる分量なら全体が見えるほうが良いとの判断で統合）。有効予約ありの場合は通常の削除確定モーダルを出さず F-1 が確定を
