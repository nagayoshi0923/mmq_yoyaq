...
- [x] F-1 削除フローの統合（**完了** 2026-06-13）: 有効予約のある公演の削除時、
      「件数つき警告 → キャンセルメール送信の選択 → 全予約一括キャンセル（記録保持）→
      公演削除＋履歴」の統合フローを handleActiveReservationsBeforeDelete として実装。
      右クリック削除・公演編集モーダルの「この予定を削除」の両方が同フローを通る。
      メール送信は reservationApi.cancel（サーバー側一括オーケストレーション）を使用。
      ダイアログは window.confirm の暫定UI（専用モーダル化は後日の磨き込みで）
