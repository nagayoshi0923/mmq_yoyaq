...
- [x] F-1 削除フローの統合（**完了** 2026-06-13）: 有効予約のある公演の削除時、
      ...
      UI は専用2ステップモーダル DeleteEventCancelDialog（予約一覧の「予約をキャンセル」
      と作法を統一）。有効予約ありの場合は通常の削除確定モーダルを出さず F-1 が確定を
      兼ねる（確定モーダルが先に出る順序は不自然、というオーナー指摘を反映）。
      **中止フローも同型に統一（2026-06-13）**: 有効予約ありの中止は同じ2ステップ
      ダイアログ（variant:'cancel'）を通り、メール送信が選択制に（従来は必ず送信・
      件数も見えなかった）。対象予約も in('confirmed','pending') → neq('cancelled') に
