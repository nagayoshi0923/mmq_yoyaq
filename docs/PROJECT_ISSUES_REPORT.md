...
**認証・セッション系**
1. **C-8 パスワードリセット失敗** - 変更時にエラー
2. **C-9 X/Discordログイン動作しない** - Googleのみ動作
3. **C-10 新規登録でメール届かない** - 確認なしでログイン可、重複登録可
4. **C-11 ログイン後に予約フローに戻れない** - 選択情報も消える

**予約・キャンセル系**
5. **C-1 残席より多い人数を予約できる** - 同時予約時の競合制御なし
6. **C-3/C-4 予約変更・キャンセルができない** - マイページ機能未動作
7. **C-14 同じ時間帯の別公演で警告なし** - 重複予約防止不十分

**カレンダー・日程系**
8. **C-6 過去の日付が選択できる** - 過去公演が表示・予約可能
9. **C-7 2月以降のカレンダー表示されない** - データ取得範囲の問題

**その他機能**
10. **C-2 キャンセル待ち登録エラー** - `permission denied for table users`
11. **C-5 お問い合わせ送信できない** - フォーム送信失敗
12. **C-12 「02てすとぴょん」PCで表示できない** - 特定シナリオのバグ
13. **C-13 PC Chromeで予約完了情報表示されない** - ブラウザ依存バグ
...
| # | カテゴリ | 問題 | 詳細 |
|---|----------|------|------|
| C-1 | 予約 | **残席より多い人数を予約できる** | 別アカウントで同時予約時、残席数を超えて販売可能（競合制御なし）|
| C-2 | キャンセル待ち | **登録時エラー** | `permission denied for table users` エラーで登録できない |
| C-3 | マイページ | **予約変更ができない** | 予約人数の変更機能が動作しない |
| C-4 | マイページ | **予約キャンセルができない** | キャンセルボタン・ダイアログ・メール通知すべて未動作 |
| C-5 | 問い合わせ | **送信できない** | 全体版・クインズ版両方で送信不可 |
| C-6 | カレンダー | **過去の日付が選択できる** | 過去の公演が表示され選択可能、シナリオ詳細でも過去日程表示 |
| C-7 | カレンダー | **2月以降が表示されない** | 月切り替え自体は動作するがデータ表示なし |
| C-8 | 認証 | **パスワードリセット失敗** | メールは届くが変更時に「失敗しました」エラー |
| C-9 | 認証 | **ソーシャルログイン（X/Discord）動作しない** | Googleのみ動作 |
| C-10 | 認証 | **新規登録でメール届かない** | 確認リンクなしでログイン可能、同メールで重複登録可、名前・電話がユーザー情報に反映されない |
| C-11 | 予約フロー | **ログイン後に予約フローに戻れない** | 選択した公演・人数も保持されない |
| C-12 | シナリオ | **「02てすとぴょん」PCで表示できない** | PC Chromeでクリック時に詳細ページ表示できない（SPは問題なし）|
| C-13 | 予約完了 | **PC Chromeで予約番号・サマリー表示されず** | SPでは表示される |
| C-14 | 重複予約 | **同じ時間帯の別公演で警告出ない** | 全く同じ時間開始でも警告なし、日付違い同シナリオでも警告なし |
...
| # | カテゴリ | 問題 | 詳細 |
|---|----------|------|------|
| L-1 | UI | **場所検索を複数選択可能に** | トップ画面で複数店舗選択したい |
| L-2 | 文言 | **「8日以降の公演を見る」→「8日後」** | 表記改善 |
| L-3 | ナビ | **スクロール位置が保持されない** | 一覧を下スクロール→詳細→戻る→一番上に戻る（改善済みだが再発？）|
| L-4 | フィルター | **ジャンルの選択方法が3箇所** | ボタン・プルダウン・検索窓で選択可能だが直感的でない |
| L-5 | カレンダー | **日付横の+1の意味が不明** | SP表示で日付横の数字の意味が分からない |
| L-6 | 貸切 | **候補日時1つしか登録できない** | カレンダー経由の場合のみ |
| L-7 | 貸切 | **シナリオ選択が大変** | 一覧が長い |
| L-8 | 貸切 | **人数選択できない** | 可変型シナリオでも選択不可（仕様確認要）|
| L-9 | 問い合わせ | **ログイン中でも名前・メール自動入力なし** | ユーザー情報があれば自動入力すべき |
| L-10 | 問い合わせ | **送信先確認画面のメッセージ違和感** | 「以下の内容でお問い合わせを送信します。」|
| L-11 | 重複予約 | **案内が画面上部でスクロール必要** | PCでスクロールしないと見えない |
| L-12 | 認証 | **Googleログイン時の表示** | 「cznpcewciwywcqcxktba.supabase.co」にログインと表示される |
