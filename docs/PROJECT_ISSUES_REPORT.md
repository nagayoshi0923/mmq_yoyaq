...
### 🟡 未対応（中優先度）
1. **7.1 大きすぎるファイルの分割** - api.ts(917行)、AuthContext.tsx(456行)
2. **6.2 throw errorの一貫性** - 部分対応済み、追加対応は低優先
3. **11.4 マイページの初回体験** - オンボーディングツアー未実装
4. **11.5 エラーハンドリングの改善** - リトライボタン、ユーザーフレンドリーメッセージ
5. **10.1 メールテンプレートのハードコード** - テンプレート管理システム化

### 🟢 未対応（低優先度）
6. **7.3 重複コードの統合** - calculateStatusBadge等の重複関数
7. **8.1 localStorage直接使用の統一** - 専用hookへの統一
8. **8.2 シナリオ編集コンポーネントの統合** - V1/V2の統合
9. **9.2 react-query-devtoolsの依存位置** - devDependenciesへ移動
10. **9.3 ESLint設定の追加** - strict mode有効化
11. **11.6 通知機能の実装** - プッシュ通知、リアルタイム更新
12. **11.7 モバイル最適化の完了** - 管理ページのレスポンシブ対応
13. **11.8 アクセシビリティの改善** - フォーカス管理、キーボード操作
14. **11.9 検索機能の改善** - フィルタリング、検索履歴
15. **11.10 ローディング状態の改善** - スケルトンローディング
16. **11.14 aria属性の追加** - WCAG対応
17. **11.15 オフライン対応（PWA）** - Service Worker
18. **11.16 予約完了後の導線改善** - 関連シナリオ提案（追加改善）
