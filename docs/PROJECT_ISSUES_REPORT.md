...
11. ~~予約フローの不明確さ~~ ✅対応完了
12. **404ページの作成**（新規）
13. **キャンセル待ち機能の実装または非表示**（新規）
14. **マイページアバター画像の永続化**（新規）
15. **エラーハンドリングの改善**
16. **プラットフォームと団体予約サイトの文脈切り替え改善**（新規）
17. **存在しない組織スラッグ時のリカバリ導線改善**（新規）
18. **予約人数UIを残席に連動**（新規）
19. **お問い合わせ導線（MMQ宛/団体宛）の明確化**（新規）
...
29. **`window.location.href` 遷移の削減（SPA体感改善）**（新規）
30. **E2EをパスURL前提へ移行**（新規）
31. **通知機能の実装**
32. **モバイル最適化の完了**
33. **検索機能の改善**
34. **ローディング状態の改善**
