...
- **status/lane:** REWORK / HIGH-RISK（独立検収で認可・tenant境界・migration意味互換の重大3件を確認。同workerへ1往復で返却）
...
- **review:** reviewer `019f91f3-983e-7723-a528-ace3c1ff196c`、target `643ff96fb010171e919e6c893c3477b2e331e8ef`、REWORK。`create_private_booking_request`はcaller/customer/group organizer照合とDB保存済み候補・希望店舗を信頼元にすること、`approve_private_booking`はadminを含む全callerへreservation organization一致を必須にすること、承認成功時はlocked reservation候補から`candidate_datetimes`のconfirmed状態をserver再構築して保存することを要求。foreign caller/group/store/candidate、他org admin、成功時confirmed stateをSQL testへ追加する。verify/unit/security/anon/JST/build/diff/mirror/production artifact除外はgreen、SQL実行は安全なdisposable DB不在でNOT RUN。
- **integration:** 未着手
