...
- **PREVIEW:** `PREVIEW_WAITING_VISUAL_OK`。監督所有Cloudflare HTTPS `https://bedding-susan-estates-healing.trycloudflare.com`を公開し、`/`、`/mypage`、`/mypage/reservation/preview-yoyaq-003`は提示直前の外部確認で各HTTP 200。workerはdesktop 1280px/mobile 390pxの横overflowなし、console error/warnなし、MyPage予約→貸切tab表示、route shellを確認。対象pure core unit 11/11、frontend eslint 0 error、diff check、RPC/migration byte一致もgreen。安全なsigned-in browser session/fixtureは存在しないため、POがHTTPS PREVIEWへ自身でsign inし、既存予約で主催者/一般メンバー、期限前/期限後/開演後、通常/貸切、ConfirmDialogの期限・料率・料金基準・金額を送信せず確認する。visual OK前は未commitのまま、final gate/REPORT/DB/Edge/staging/mainを行わない。一時proxy/tunnelはreview後に停止する。
- **REPORT:** 未着手
- **review:** 未着手
- **integration:** 未着手
