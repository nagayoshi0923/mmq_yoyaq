...
|---|---|---|---|
| `019f779d-7170-7752-a846-37c593cf3ec8` | `/private/tmp/yoyaq-002-integration` | `0d3cf4135fc8218faf13952d79a3a3f2c85d1bf1` | 2026-07-23 20:15 JST |
...
| YOYAQ-004 | 募集停止枠の貸切申請・承認を一貫して拒否 | PREVIEW_WAITING_VISUAL_OK | HIGH-RISK | 必須 / PO判断待ち | なし（YOYAQ-003と非重複で並行可、staging統合は直列） | worker `019f8e6f-2935-7143-b7b2-a03841f62e9f` / review 未着手 | - | worktree `/Users/mai/.codex/worktrees/1460/mmq_yoyaq-1`、branch `codex/yoyaq-004-blocked-private-booking`、port `5186` |
...
- **status/lane:** PREVIEW_WAITING_VISUAL_OK / HIGH-RISK（予約在庫、RPC/migration、organization/store境界、日付/time_slot、並行更新）
...
- **PREVIEW:** `PREVIEW_WAITING_VISUAL_OK`。旧URL失効後に再発行した監督所有の一時HTTPS `https://speaks-importantly-lovers-kills.trycloudflare.com/yoyaq-004-preview.html`で認証不要・実データ非使用・PIIなし・送信不能fixtureを公開。`state=all-blocked`、`mixed-stores`、`stale-submit`、`blocked-after-request`、`blocked-at-request`の5直linkを提示直前に外部HTTP 200と実ブラウザで再確認し、desktop 1280px/mobile 390pxとも横overflow 0、console error/warn 0、native dialogなし。全店舗停止はdisabled＋「現在受付停止中」、一部店舗受付中は選択可、送信直前停止は日付/time_slot/店舗付き再選択エラー、申請後停止と申請時点停止（既存不整合）を区別し、いずれも承認disabledかつ申請データを保持する。PO visual OK前は未commitで、最終gate/REPORT/review/DB/Edge/staging統合を行わない。production artifact除外はvisual OK後の最終gateで確認する。
