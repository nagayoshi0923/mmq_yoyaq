...
### リリース可否: ✅ YES — P0 全3件修正済み・P1 全10件修正済み・staging 検証合格（2026-02-10）
### 残条件: Sentry DSN の Vercel 環境変数設定、本番 DB へのマイグレーション適用、ステージング最終動作確認

| 質問 | 回答 | 根拠 |
|------|------|------|
| 1) 第三者が悪意なく触っても壊れないか | **YES** | 楽観的ロック追加、Mass Assignment 防止、通知のべき等性チェック実装済み |
| 2) 将来の開発者が仕様を忘れても事故らないか | **条件付きYES** | CI に RLS 検証・セキュリティ関数上書き検出追加。監査トリガーで操作追跡可能。ロールバックガイド整備済み |
| 3) この状態で本番リリースしてよいか | **条件付きYES** | P0 全修正・P1 全修正済み。ステージングでの最終動作確認と本番DBへのマイグレーション適用が残条件 |
...
### P1-14: 新規組織作成時に必要な初期データが自動生成されない

| 項目 | 内容 |
|------|------|
| ファイル | `src/pages/OrganizationRegister/index.tsx` |
| シナリオ | 新組織を作成しても `organization_settings`, `global_settings`, 臨時会場ストア（5件）が作成されず、スケジュール画面で406エラー、臨時会場追加不可になる |
| 確認済み | テスト組織(`b0000000-...02`)で再現・手動データ投入で解消を確認 |
| 修正 | `OrganizationRegister` の組織作成フローまたは DB トリガーで、`organization_settings`, `global_settings`, 臨時会場ストア×5 を自動作成する |
...
| # | 項目 | ステータス | 対応内容 |
|---|------|----------|---------|
| P1-1 | Mass Assignment | ✅ 修正済み | staffApi/customerApi/storeApi にフィールドホワイトリスト実装 |
| P1-6 | 楽観的ロック | ✅ 修正済み | scheduleApi.update に updated_at ベースの楽観的ロック追加 |
| P1-7 | キャンセル通知 | ✅ 対応済み | 既存のリトライキュー（waitlist_notification_queue）で対応済み |
| P1-8 | メールべき等性 | ✅ 修正済み | idempotencyKey をキャンセル確認メール・ウェイトリスト通知に追加 |
| P1-9 | 関数上書きリスク | ✅ 修正済み | CI でセキュリティ重要関数の重複定義を検出する仕組み追加 |
| P1-10 | CI RLS 検証 | ✅ 修正済み | check-security-guardrails.mjs に RLS 有効化チェック・危険パターン検出追加 |
| P1-11 | 監査ログ不足 | ✅ 修正済み | schedule_events/stores/scenarios に汎用監査トリガー追加 |
| P1-12 | 相関ID | ✅ 修正済み | generateCorrelationId/createCorrelatedLogger ユーティリティ追加、キャンセルフローで使用 |
| P1-13 | ロールバック手段 | ✅ 修正済み | ROLLBACK_GUIDE.md に各マイグレーションのロールバックSQL記載 |
| P1-14 | 組織初期データ | ✅ 修正済み | organizations INSERT 後トリガーで organization_settings/global_settings/臨時会場自動生成 |
