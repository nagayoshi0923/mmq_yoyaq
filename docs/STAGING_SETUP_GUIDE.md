...
> **重要: 本番デプロイ前の確認必須**
> 
> ステージングにデプロイした後、**必ずユーザーの確認を待ってから**本番（main）にマージすること。
> AIエージェントが勝手に本番までデプロイしないよう、ステージングで一旦停止すること。
...
5. ステージング環境で動作確認 ⚠️ ここで必ず停止
   - Preview URLで確認
   - ユーザー（管理者）の承認を待つ

6. 承認後、staging → main にマージ
   - 本番環境に自動デプロイ
```

> **AIエージェントへの指示**
> 
> ステージングへのデプロイ後は、必ずユーザーに確認を求めて停止すること。
> 「ステージングで動作確認してください」と伝え、ユーザーからの「OK」「問題なし」等の
> 明示的な承認があるまで、mainへのマージや本番デプロイを行わないこと。
