...
- `api/store-dashboard.ts` の取消処理は、リクエスト本文のスタッフIDを信用せず、認証済みユーザーの `staff.user_id` から対象スタッフを解決する。
- 削除対象は `organization_id` が一致するログイン中スタッフの、JST当日かつ未退勤の `staff_checkins` に限定する。
- 取消後は店舗ダッシュボードを再取得し、未打刻状態から再打刻できることを維持する。
