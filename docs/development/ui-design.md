...
- 表示対象: `isStoreRepresentative === true` の店舗代表だけ
- 配置: 画面右下の固定カード。店舗ダッシュボード本体から独立した局所ErrorBoundary内
- loading: 「出勤打刻を確認中...」
- 未打刻: 「出勤打刻がまだです」＋クライアント現在時刻`HH:MM`の「出勤打刻する」ボタン
- 打刻済み: 「出勤打刻済み（HH:MM）」＋常時可視の「取り消す」ボタン
- API失敗/不正レスポンス: エラー文と「もう一度確認する」を表示し、ページ本体は維持
- unavailable: 利用不可メッセージを安全に表示
- 取消確認: 共通`ConfirmDialog`を1枚使用。native `confirm()`は使わない
- 退勤ボタン、カード左ボーダーアクセントは使用しない
...
- **編集モーダル** (`StoreEditModal.tsx`)
  ...
  - 追加項目: アクセス方法（最寄り駅・徒歩分数・ビル階数などを自由入力）
  - 保存/キャンセルボタン
...
## 更新履歴
- 2026-04-03: 店舗設定に「アクセス方法」入力欄を追加（`StoreBasicSettings` / `StoreEditModal`）
- 2026-01-03: マイページ/予約履歴にキャンセル・人数変更ダイアログ追加。予約確認画面に重複予約警告UI追加
- 2025-09-24: 初版作成
...
- 2026-01-01: 設定ページに給与設定タブ（SalarySettings）追加。GM給与計算式（通常公演：基本給+時給×時間、GMテスト：時給×時間、受付：固定給）を設定可能。計算例のリアルタイムプレビュー付き。
- 2026-04-03: 設定ページにメール配信履歴タブを追加。Resendの送信履歴（送信先・件名・ステータス・送信時刻）を管理画面で検索・確認可能に。
