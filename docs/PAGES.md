...
ログイン中の顧客は、PublicBookingTop・PlatformTop・ScenarioDetailPage・ScenarioDetailGlobal の体験済みチェックを再度押すと未体験へ戻せる。解除状態は `customer_played_overrides` に保存し、予約由来・手動登録由来の履歴自体は削除しない。再び体験済みにする場合は override を削除して既存履歴を復帰し、手動履歴を重複追加しない。
