...
| ページ名 | パス | 用途 | アクセス |
|---------|------|------|---------|
| **AuthorDashboard** | `#author-dashboard` | 作者ダッシュボード | 作者 |
| **AuthorRegister** | `#author-register` | 作者アカウント登録 | ログイン必須 |
...
│  3. 設定                                                         │
│     - プロフィール編集（表示名、自己紹介、SNS等）                │
│     - 通知設定（新規報告通知、定期サマリー）                     │
│                                                                  │
│  関連テーブル:                                                   │
│  - authors                                                       │
│  - scenarios (author_id or author 名で紐付け)                    │
│  - external_performance_reports                                  │
...
│                                                                  │
│  前提条件:                                                       │
│  - ログイン必須（未ログインの場合は #login へリダイレクト）      │
│  - 既に作者アカウントがある場合は #author-dashboard へリダイレクト│
│                                                                  │
│  入力項目:                                                       │
│  - 著者名/ペンネーム（必須）                                     │
│  - 表示名（オプション）                                          │
│  - 自己紹介（オプション）                                        │
│  - メールアドレス（ログイン中のアカウントから自動取得）          │
│                                                                  │
│  登録完了後:                                                     │
│  - authors テーブルに登録                                        │
│  - 3秒後に #author-dashboard へリダイレクト                      │
│                                                                  │
│  関連テーブル:                                                   │
│  - authors                                                       │
│  - auth.users                                                    │
...
| 日付 | 変更内容 | 担当 |
|------|---------|------|
| 2024-12-17 | 作者ポータル追加（AuthorDashboard, AuthorRegister）。作者が公演報告を確認・管理可能に | AI |
