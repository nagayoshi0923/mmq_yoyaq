...
| 項目 | 入力内容 |
|------|----------|
| アプリ名 | `MMQ`（または組織・サービス名） |
| ユーザーサポートメール | あなたのメールアドレス |
| アプリのロゴ | （任意）MMQのロゴをアップロード |
| アプリのホームページ | `https://mmq.game`（本番） |
| アプリのプライバシーポリシーリンク | `https://mmq.game/privacy` |
| アプリの利用規約リンク | `https://mmq.game/terms` |
...
| 名前 | `MMQ Web Client` |
| 承認済みの JavaScript 生成元 | `https://mmq.game`（本番）<br>（`https://www.mmq.game` を使う場合はその URL も追加）<br>`http://localhost:5173`（開発用） |
...
| 項目 | 入力内容 |
|------|----------|
| Name | `MMQ`（または組織・サービス名） |
| Description | マーダーミステリー予約プラットフォーム |
| App Icon | （任意）MMQのロゴをアップロード |
| Terms of Service URL | `https://mmq.game/terms` |
| Privacy Policy URL | `https://mmq.game/privacy` |
...
## 📝 本番環境チェックリスト（`https://mmq.game`）

- [ ] **Supabase** → Authentication → **URL Configuration**
  - **Site URL**: `https://mmq.game`（`www` 運用なら `https://www.mmq.game` に統一）
  - **Redirect URLs**: `https://mmq.game/**` を含める（ワイルドカード可なら）。`www` から配信する場合は `https://www.mmq.game/**` も追加
- [ ] **Google Cloud Console** → 認証情報 → OAuth クライアント
  - **承認済みの JavaScript 生成元**に `https://mmq.game`（および使うなら `https://www.mmq.game`）
  - **承認済みのリダイレクト URI**は **`https://<project-ref>.supabase.co/auth/v1/callback` のみ**（フロントの mmq.game はここには書かない）
- [ ] Google OAuth 同意画面を「本番環境」に公開（必要に応じて）
- [ ] Discord Developer Portal で本番用の Redirect URL を追加
- [ ] Vercel（等）の **`VITE_SUPABASE_URL` / Key** が本番プロジェクト向きか確認
