...
PR431からの一括取込を避け、営業時間設定の特別営業日・休業日の読込と保存を修正。旧holidaysの保持、保存失敗の安全停止、組織・店舗絞り込み、共通処理の抽出を実施。unit370件・verify・模擬DBによる画面試験3件成功。本番未反映。店舗編集への集約、募集停止、貸切時刻、GM解除・取消連動は残る。詳細：`docs/releases/QW-20260909-011-store-settings.md`。
