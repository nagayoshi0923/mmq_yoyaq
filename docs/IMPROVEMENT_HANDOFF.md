...
- [ ] 本番配信の最終確認と表示修正（期限切れの利用可能欄への残留、併用条件の固定文言）を同案件で完了する。詳細 `docs/COUPON_RULES.md`。
...
- [ ] 追加余白の本番反映後に表示を確認し、会社Driveの同案件へ証跡を追記。
...
- [ ] DB migration `20260924120000_common_recruitment_targets.sql` をステージング→本番の順で適用。本番は利用者の明示指示後。逆DDLは `supabase/rollbacks/20260924120000_common_recruitment_targets.sql`。新設定/履歴を退避してから使用。
- [ ] 本番反映後【設定→運用と案内→開催判断・追加募集】割合変更で計算例が更新されること。【シナリオ編集→ゲーム設定】共通/個別の切替と保存を確認。
...
- [ ] main反映・本番配備。前件のstaging固定URLキャッシュ問題は別途承認待ち。今回のコードは前件のDB移行に依存しない。
...
- [ ] 検証環境の画面確認：売上管理→スタッフ給与計算、売上概要/FC売上で月途中改定の前後を表示し、合計・明細・受付0円を確認。履歴取得失敗時は金額が非表示で理由と再試行ボタンが出ることを確認。
- [ ] main反映・本番配備。DB変更なし。第2段階のGM staff_id移行は未着手。

既存のシナリオ固有報酬・交通費・GM未配置時の見込計上などのルールは今回変更していない。売上の店舗/作品ランキングでシナリオ固有gm_costsがない場合のGM費用が計上されない既存処理は、集計方式統一の別途検討事項として残る。
