...
- **SEC-P2-01**: URL由来ID参照（予約詳細）でID試行・列挙がしやすい（RLSで止まる前提だがノイズ/ログ汚染） → **✅ 修正完了（エラーを統一）**
...
- **推奨対策**
  - ✅ 実施済み: エラーを「存在しない/権限なし」で分岐せず統一（`maybeSingle` + 一律メッセージ）
...
- **推奨対策**
  - ✅ 実施済み: 予約作成は同一送信内で `reservation_number` を固定し、UNIQUE衝突時は既存予約を取得して1件に収束
  - ✅ 実施済み: 予約確認メールは `booking_email_queue` を `reservation_id + email_type` で一意化し、`completed` は二重送信をスキップ
  - Runbook: `docs/deployment/SEC_P1_XX_IDEMPOTENCY_RUNBOOK.md`
