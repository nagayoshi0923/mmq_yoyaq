ALTER TABLE public.reservations ADD COLUMN reservation_change_deadline_hours_snapshot integer CHECK(reservation_change_deadline_hours_snapshot >= 0);
-- 既存予約へ新しい期限を遡及適用しない。新規申込時の時間数を固定する。
...
CREATE TRIGGER reservation_change_policy_snapshot BEFORE INSERT OR UPDATE ON public.reservations FOR EACH ROW EXECUTE FUNCTION public.set_reservation_change_policy_snapshot();
