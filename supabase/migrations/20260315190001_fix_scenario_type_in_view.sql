-- organization_scenarios に scenario_type カラムを追加
ALTER TABLE public.organization_scenarios
ADD COLUMN IF NOT EXISTS scenario_type TEXT DEFAULT 'normal';

COMMENT ON COLUMN public.organization_scenarios.scenario_type IS 'シナリオタイプ（normal: 通常, managed: 管理作品）';

-- 既存のscenariosテーブルからscenario_typeをコピー
UPDATE public.organization_scenarios os
SET scenario_type = s.scenario_type
FROM public.scenarios s
WHERE s.scenario_master_id = os.scenario_master_id
  AND s.organization_id = os.organization_id
  AND s.scenario_type IS NOT NULL
  AND s.scenario_type != 'normal';

-- ビューを再作成して scenario_type を organization_scenarios から取得
...
SELECT
  ...
  -- 担当GM: staff_scenario_assignments から動的に取得
  COALESCE(
  ...
  os.pricing_patterns,
  TRUE AS is_shared,
  -- scenario_type を organization_scenarios から取得（修正）
...
COMMENT ON VIEW public.organization_scenarios_with_master IS 
  '組織シナリオとマスタを結合したビュー。scenario_typeをorganization_scenariosから取得';
