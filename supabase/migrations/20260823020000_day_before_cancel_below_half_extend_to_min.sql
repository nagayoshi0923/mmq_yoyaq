-- 前日判定の中止ラインを過半数に戻す。開催決定は最低開催人数のまま。
--   最低開催人数以上 → confirmed（開催決定。募集受付は継続）
--   過半数以上・最低人数未満 → extended（4時間前まで募集延長）
--   過半数未満 → cancelled
-- 20260821160000 は「min未満なら中止」にしており、4人公演 3/4 が中止になる設定ミスだった。
-- 破壊的変更なし（CREATE OR REPLACE のみ）。ロールバックは 20260821160000 の day_before 定義。
...
'前日23:59に実行する公演中止判定（実予約＋未同期スタッフ・organization_scenariosの定員反映）。最低開催人数以上=開催確定（募集継続）/ 過半数以上=募集延長 / 過半数未満=中止。p_target_dateを指定すると任意の日付を対象にできる。';
