ALTER TABLE public.global_settings ADD COLUMN booking_cutoff_minutes integer NOT NULL DEFAULT 0 CHECK (booking_cutoff_minutes BETWEEN 0 AND 1440);
ALTER TABLE public.organization_recruitment_settings ADD COLUMN enabled boolean NOT NULL DEFAULT true, ADD COLUMN deadline_minutes integer NOT NULL DEFAULT 90 CHECK (deadline_minutes BETWEEN 1 AND 239);
ALTER TABLE public.organization_scenarios ADD COLUMN recruitment_enabled_source text NOT NULL DEFAULT 'common' CHECK (recruitment_enabled_source IN ('common','custom')), ADD COLUMN recruitment_deadline_source text NOT NULL DEFAULT 'common' CHECK (recruitment_deadline_source IN ('common','custom'));
-- Preserve non-default individual choices; unchanged default values inherit equivalent common defaults.
UPDATE public.organization_scenarios SET recruitment_enabled_source='custom' WHERE recruitment_extension_enabled IS DISTINCT FROM true;
UPDATE public.organization_scenarios SET recruitment_deadline_source='custom' WHERE recruitment_deadline_minutes IS DISTINCT FROM 90;
UPDATE public.schedule_events e SET booking_cutoff_minutes=GREATEST(e.reservation_deadline_hours,0)*60 WHERE e.booking_cutoff_minutes IS NULL AND e.reservation_deadline_hours>0 AND NOT EXISTS (SELECT 1 FROM public.organization_scenarios os WHERE os.organization_id=e.organization_id AND os.booking_cutoff_minutes IS NOT NULL AND ((e.organization_scenario_id IS NOT NULL AND os.id=e.organization_scenario_id) OR (e.organization_scenario_id IS NULL AND os.scenario_master_id=COALESCE(e.scenario_master_id,e.scenario_id))));
