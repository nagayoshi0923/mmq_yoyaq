-- scenario_masters に report_display_name カラムを追加
ALTER TABLE public.scenario_masters
ADD COLUMN IF NOT EXISTS report_display_name TEXT;

COMMENT ON COLUMN public.scenario_masters.report_display_name IS '報告用表示名（作者名とは別に報告書で使用する名前）';

-- organization_scenarios にも追加（既に存在する場合はスキップ）
ALTER TABLE public.organization_scenarios
ADD COLUMN IF NOT EXISTS report_display_name TEXT;

COMMENT ON COLUMN public.organization_scenarios.report_display_name IS '報告用表示名（組織ごとにカスタマイズ可能）';

-- ビューを更新して report_display_name を含める
...
SELECT
  ...
  -- 報告用表示名: org優先 → master → author にフォールバック
...
COMMENT ON VIEW public.organization_scenarios_with_master IS 
  '組織シナリオとマスタを結合したビュー。report_display_name対応';

DO $$ 
BEGIN
  RAISE NOTICE '✅ report_display_name カラムを追加しました';
END $$;
