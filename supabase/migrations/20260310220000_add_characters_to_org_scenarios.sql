-- organization_scenarios にキャラクター情報を追加
-- 作成日: 2026-03-10

-- 1. organization_scenarios テーブルに characters カラムを追加
ALTER TABLE public.organization_scenarios
ADD COLUMN IF NOT EXISTS characters JSONB DEFAULT '[]'::JSONB;

COMMENT ON COLUMN public.organization_scenarios.characters IS 'キャラクター情報（配列）: [{name, gender, age, occupation, description, sort_order}]';

-- 2. ビューを再作成して characters を含める
...
COMMENT ON VIEW public.organization_scenarios_with_master IS 
  '組織シナリオとマスタを結合したビュー。キャラクター情報を追加';

-- 完了通知
DO $$
BEGIN
  RAISE NOTICE '✅ マイグレーション完了: organization_scenarios にキャラクター情報を追加';
END $$;
