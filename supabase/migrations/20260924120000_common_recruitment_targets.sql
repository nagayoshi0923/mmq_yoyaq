-- QW-20260921-004: 共通/個別の不足人数・割合指定（端数切り捨て）
BEGIN;
...
-- 従来の個別値と設定履歴のある作品は、共通値と同じ値でも個別指定を維持。
UPDATE public.organization_scenarios s SET recruitment_target_value=s.recruitment_max_missing,
 recruitment_target_source=CASE WHEN s.recruitment_max_missing<>2 OR EXISTS(SELECT 1 FROM public.scenario_recruitment_setting_history h WHERE h.organization_id=s.organization_id AND h.organization_scenario_id=s.id) THEN 'custom' ELSE 'common' END;
