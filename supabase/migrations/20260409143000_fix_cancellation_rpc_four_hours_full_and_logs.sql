-- =============================================================================
-- マイグレーション: 中止判定RPCの退行修正（4時間前・前日）
-- =============================================================================
--
-- 問題（20260409110000 の退行）:
--   1) check_performances_four_hours_before が IF v_current >= 1 となり、
--      募集延長公演が「1人以上いれば開催確定」になっていた（満席判定が無効化）。
--   2) 中止時の INSERT が participants_count を参照（テーブルは current_participants）。
--   3) 開催確定時に performance_cancellation_logs を書かず、
--      NOT EXISTS 条件のため同一公演が毎時チェックされ続ける可能性。
--   4) check_performances_day_before が「満席 / 過半数延長 / 過半数未満中止」の
--      3分岐を失い、>= 半分を一律 confirmed にしていた。
--
-- 修正:
--   - 070800 の判定ロジックに戻しつつ、091100 の max 解決（override 優先）を維持
-- =============================================================================
...
'前日23:59に実行する公演中止判定（実予約＋未同期スタッフ・organization_scenariosの定員反映）';
