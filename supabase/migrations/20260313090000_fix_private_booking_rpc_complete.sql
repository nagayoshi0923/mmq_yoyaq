-- =============================================================================
-- 20260313090000: create_private_booking_request RPC の完全修正
-- =============================================================================
-- 前回のマイグレーション (20260313080000) が不完全だったため再作成
--
-- 問題:
-- - sm.base_price と sm.participation_fee を参照していたが、
--   scenario_masters テーブルにはこれらのカラムが存在しない
-- - participation_fee は organization_scenarios テーブルにある
-- - reservations テーブルに必要なカラムが不足している
--
-- 修正:
-- - 不足しているカラムを追加
-- - participation_fee は organization_scenarios (os) から取得
-- - scenario_title は title カラムに変更
--
-- =============================================================================
...
ALTER TABLE reservations ADD COLUMN IF NOT EXISTS candidate_datetimes JSONB;
ALTER TABLE reservations ADD COLUMN IF NOT EXISTS reservation_type TEXT DEFAULT 'normal';
...
BEGIN
  ...
  -- scenarios テーブルから取得し、organization_scenarios から料金を取得
  SELECT 
    s.id,
    s.title,
    s.organization_id,
  ...
  -- 最初の候補日時を取得（requested_datetime に設定）
  ...
  -- 予約を作成（reservations テーブルの既存カラムに合わせる）
  ...
  -- staff_scenario_assignments.scenario_id は scenario_master_id または scenario.id の場合がある
  FOR v_gm_id IN
    SELECT ssa.staff_id
...
DO $$
BEGIN
  RAISE NOTICE 'create_private_booking_request RPC のカラム参照エラーを修正しました';
END $$;
