-- =============================================================================
-- マイグレーション: create_reservation_with_lock_v2 を organization_scenarios 対応に更新
-- =============================================================================
-- 
-- 作成日: 2026-03-10
-- 
-- 変更:
--   scenarios テーブルではなく organization_scenarios テーブルから
--   料金情報を取得するように変更。クーポン機能も維持。
-- 
-- =============================================================================

-- 既存の関数を全て削除（CASCADE で全オーバーロード削除）
DO $$
DECLARE
  func_record RECORD;
BEGIN
  -- create_reservation_with_lock_v2 の全バージョンを削除
  FOR func_record IN 
  ...
  -- create_reservation_with_lock の全バージョンを削除
  FOR func_record IN 
    SELECT p.oid::regprocedure::text as func_sig
    FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
...
BEGIN
  ...
  -- 料金計算（サーバー側）
  -- 1. organization_scenario_id があれば organization_scenarios から取得
  -- 2. なければ scenario_id で scenarios_v2 ビュー（または scenarios）から取得
  ...
  ELSIF v_scenario_id IS NOT NULL THEN
    -- scenarios_v2 ビューを試す（organization_scenarios + scenario_masters 結合）
    SELECT participation_fee, participation_costs, duration, title
    INTO v_participation_fee, v_participation_costs, v_duration, v_title
    FROM scenarios_v2
    WHERE id = v_scenario_id;
    
    -- scenarios_v2 で見つからなければ旧 scenarios テーブルを試す
    IF NOT FOUND THEN
      SELECT participation_fee, participation_costs, duration, title
      INTO v_participation_fee, v_participation_costs, v_duration, v_title
      FROM scenarios
  ...
  IF p_customer_coupon_id IS NOT NULL THEN
    ...
    -- 対象チェック（scenario_id または org_scenario_id で判定）
    ...
    ELSIF v_campaign.target_type = 'specific_scenarios' THEN
      -- scenario_id または org_scenario_id のどちらかが対象に含まれればOK
...
'予約作成（v2 + クーポン + organization_scenarios対応）。organization_scenario_id を優先して料金取得。';

-- =====================
-- 旧版の create_reservation_with_lock も organization_scenarios 対応に更新
-- =====================
...
'予約作成（v1互換ラッパー）。内部でv2を呼び出す。';

-- 完了確認
DO $$ 
BEGIN
  RAISE NOTICE '✅ マイグレーション完了: 予約作成RPC を organization_scenarios 対応に更新';
  RAISE NOTICE '   - organization_scenario_id を優先';
  RAISE NOTICE '   - scenario_id で scenarios_v2 ビューを使用';
  RAISE NOTICE '   - 旧 scenarios テーブルにフォールバック';
  RAISE NOTICE '   - クーポン機能維持';
END $$;
