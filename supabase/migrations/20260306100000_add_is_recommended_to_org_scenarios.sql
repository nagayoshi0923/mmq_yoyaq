-- organization_scenarios に is_recommended カラムを追加
-- organization_scenarios_with_master ビューにも追加
-- 作成日: 2026-03-06

-- 1. organization_scenarios テーブルに is_recommended カラムを追加
ALTER TABLE public.organization_scenarios
ADD COLUMN IF NOT EXISTS is_recommended BOOLEAN DEFAULT FALSE;

COMMENT ON COLUMN public.organization_scenarios.is_recommended IS '管理者が設定するおすすめフラグ';

-- 2. ビューを再作成して is_recommended を含める
...
COMMENT ON VIEW public.organization_scenarios_with_master IS 
  '組織シナリオとマスタを結合したビュー。is_recommendedカラムを追加';
