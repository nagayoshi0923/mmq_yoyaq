-- =============================================================================
-- 20260130233000: 予約制限をDB/RPC側で強制（SEC-P1-01）
-- =============================================================================
--
-- 目的:
-- - フロントの予約制限チェックはUX目的に留め、最終防衛はDBでfail-closedにする
-- - 通信/設定取得エラーや直叩きでも、締切/上限/件数制限が素通りしないようにする
--
-- 対象:
...
-- -----------------------------------------------------------------------------
-- v2: create_reservation_with_lock_v2
-- -----------------------------------------------------------------------------
...
DECLARE
  ...
  v_date DATE;
  v_start_time TIME;
  v_reservation_deadline_hours INTEGER;
  v_duration INTEGER;
  v_title TEXT;
  ...
  -- 予約制限（store設定）
  v_max_participants_per_booking INTEGER;
  v_advance_booking_days INTEGER;
  v_same_day_booking_cutoff INTEGER;
  v_max_bookings_per_customer INTEGER;
...
BEGIN
  ...
  INTO v_event_org_id, v_scenario_id, v_store_id, v_date, v_start_time, v_reservation_deadline_hours, v_max_participants
  ...
  IF v_hours_until_event < 0 THEN
    RAISE EXCEPTION 'EVENT_ALREADY_STARTED' USING ERRCODE = 'P0036';
  END IF;

  IF v_reservation_deadline_hours IS NOT NULL AND v_hours_until_event < v_reservation_deadline_hours THEN
    RAISE EXCEPTION 'RESERVATION_DEADLINE_PASSED' USING ERRCODE = 'P0037';
  END IF;
  ...
  IF v_store_id IS NOT NULL THEN
    SELECT
      ...
      max_bookings_per_customer
    INTO
      v_max_participants_per_booking,
      v_advance_booking_days,
      v_same_day_booking_cutoff,
      v_max_bookings_per_customer
    FROM reservation_settings
    WHERE store_id = v_store_id
      AND (organization_id = v_event_org_id OR organization_id IS NULL)
    ORDER BY CASE WHEN organization_id = v_event_org_id THEN 1 ELSE 0 END DESC, organization_id NULLS LAST
    LIMIT 1;

    IF v_max_participants_per_booking IS NOT NULL AND p_participant_count > v_max_participants_per_booking THEN
      RAISE EXCEPTION 'MAX_PARTICIPANTS_PER_BOOKING' USING ERRCODE = 'P0034';
    END IF;

    IF v_advance_booking_days IS NOT NULL AND v_date > (CURRENT_DATE + v_advance_booking_days) THEN
      RAISE EXCEPTION 'ADVANCE_BOOKING_LIMIT' USING ERRCODE = 'P0033';
    END IF;

    IF v_same_day_booking_cutoff IS NOT NULL AND v_date = CURRENT_DATE AND v_hours_until_event < v_same_day_booking_cutoff THEN
      RAISE EXCEPTION 'SAME_DAY_CUTOFF_PASSED' USING ERRCODE = 'P0038';
    END IF;

    IF v_max_bookings_per_customer IS NOT NULL AND v_max_bookings_per_customer > 0 THEN
      SELECT COUNT(*)
      ...
      WHERE r.customer_id = p_customer_id
        ...
        AND se.organization_id = v_event_org_id
        AND se.date = v_date;

      IF v_booking_count >= v_max_bookings_per_customer THEN
        RAISE EXCEPTION 'MAX_BOOKINGS_PER_CUSTOMER' USING ERRCODE = 'P0035';
      END IF;
    END IF;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.create_reservation_with_lock_v2 TO authenticated;

-- -----------------------------------------------------------------------------
-- 互換維持版: create_reservation_with_lock（既存シグネチャ維持）
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT organization_id,
         ...
         reservation_deadline_hours,
         COALESCE(max_participants, capacity, 8)
  INTO v_event_org_id, v_event_scenario_id, v_event_store_id, v_date, v_start_time, v_reservation_deadline_hours, v_max_participants
  FROM schedule_events
  WHERE id = p_schedule_event_id
    AND is_cancelled = false
  FOR UPDATE;
...
'予約作成（互換維持版）。料金/日時はサーバー側で確定し、予約制限（締切/上限/件数）をDBで強制する（SEC-P1-01）。';
