...
ADD COLUMN IF NOT EXISTS reservation_change_note TEXT DEFAULT '参加人数の変更は、マイページから公演開始24時間前まで無料で行えます。日程の変更をご希望の場合は、一度キャンセルの上、再度ご予約をお願いいたします。この場合、キャンセル時期によってキャンセル料が発生する場合があります。',
ADD COLUMN IF NOT EXISTS private_reservation_change_deadline_hours INTEGER DEFAULT 168,
ADD COLUMN IF NOT EXISTS private_reservation_change_note TEXT DEFAULT '貸切予約の変更は、公演開始1週間前まで可能です。日程変更は空き状況によります。',
...
COMMENT ON COLUMN public.reservation_settings.reservation_change_deadline_hours IS '通常公演の予約変更可能期限（時間前）';
COMMENT ON COLUMN public.reservation_settings.reservation_change_note IS '通常公演の予約変更に関する説明';
COMMENT ON COLUMN public.reservation_settings.private_reservation_change_deadline_hours IS '貸切公演の予約変更可能期限（時間前）';
COMMENT ON COLUMN public.reservation_settings.private_reservation_change_note IS '貸切公演の予約変更に関する説明';
