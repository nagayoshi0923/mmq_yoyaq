-- 前日判定の募集延長ラインを「定員の過半数」から「最低開催人数の半分」に変更する。
--   >= v_min → confirmed
--   >= CEIL(v_min / 2) かつ < v_min → extended
--   < CEIL(v_min / 2) → cancelled
-- 破壊的変更なし（関数の差し替えのみ）。
-- ロールバック: v_half を CEIL(v_max / 2) に戻す。
-- live pg_get_functiondef を正に、v_half の1行だけ変更。
...
BEGIN
  ...
  LOOP
    ...
    -- 開催決定は最低開催人数。中止判断は最低開催の半分（半分以上・最低人数未満は募集延長）
...
'前日23:59に実行する公演中止判定（実予約＋未同期スタッフ・organization_scenariosの定員反映）。最低開催人数以上=開催確定（募集継続）/ 最低開催の半分以上=募集延長 / 半分未満=中止。p_dry_run=TRUE なら書き込みしない（21:00予告）。p_target_dateを指定すると任意の日付を対象にできる。';
