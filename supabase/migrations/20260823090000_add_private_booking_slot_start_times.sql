-- シナリオごとの貸切開始時刻（平日/土日祝 × 朝/昼/夜）
-- 欠落キーは店舗の営業時間設定 slot_start_times を使う
--
-- ロールバック:
--   1) ビューを本ファイル直前の live 定義（is_license_buyout まで）に戻す
--   2) ALTER TABLE public.organization_scenarios DROP COLUMN IF EXISTS private_booking_slot_start_times;
...
COMMENT ON COLUMN public.organization_scenarios.private_booking_slot_start_times IS
  '貸切の朝/昼/夜開始時刻。{"weekday":{"morning":"09:00"},"weekend":{...}}。欠落キーは店舗営業時間設定';
