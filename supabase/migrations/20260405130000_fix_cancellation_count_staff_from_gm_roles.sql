-- =============================================================================
-- マイグレーション: 中止判定でスタッフ参加者（gm_roles）も参加者数に含める
-- =============================================================================
--
-- 問題:
--   スタッフ予約（reservation_source='staff_entry'）はフロントエンドで
--   スケジュール画面を開いた際に初めてDBに作成される。
--   前日23:59のチェック時点ではスタッフ予約がまだ存在しないため、
--   実際はスタッフ参加で満席なのに「人数未達」で中止判定されるケースがあった。
--
-- 修正:
--   reservationsテーブルの実予約に加え、schedule_events.gm_roles から
--   role='staff' のスタッフ（予約レコード未作成分）もカウントする。
--   二重カウント防止: 既にreservationが存在するスタッフは除外する。
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- gm_rolesでスタッフ参加だが、まだ予約レコードが作られていない人数をカウント
    SELECT COUNT(*)
    INTO v_unsynced_staff
    FROM jsonb_each_text(COALESCE(v_event.gm_roles, '{}'::jsonb)) AS kv(name, role)
    WHERE kv.role = 'staff'
      AND NOT EXISTS (
        ...
        WHERE r2.schedule_event_id = v_event.id
          ...
          AND r2.participant_names @> jsonb_build_array(kv.name)
      );
