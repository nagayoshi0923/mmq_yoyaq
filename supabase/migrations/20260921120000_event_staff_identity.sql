...
LOCK TABLE public.schedule_events IN SHARE ROW EXCLUSIVE MODE;
...
-- Preserve every original position, including unregistered names and duplicates.
INSERT INTO public.schedule_event_staff_assignments
SELECT e.id, g.ordinality::integer, e.organization_id,
  CASE WHEN d.n = 1 THEN s.id END, g.name,
  coalesce(nullif(e.gm_roles->>g.name,''),CASE WHEN g.ordinality=1 THEN 'main' ELSE 'sub' END),
  CASE WHEN d.n > 1 THEN 'duplicate' WHEN s.id IS NULL THEN 'unmatched' ELSE 'resolved' END
FROM public.schedule_events e CROSS JOIN LATERAL unnest(e.gms) WITH ORDINALITY g(name,ordinality)
LEFT JOIN public.staff s ON s.organization_id=e.organization_id AND s.name=g.name
CROSS JOIN LATERAL (SELECT count(*) n FROM unnest(e.gms) x(name) WHERE x.name IS NOT DISTINCT FROM g.name) d;
