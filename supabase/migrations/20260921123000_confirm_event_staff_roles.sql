-- Keep uncertain legacy roles out of payroll rather than infer a renamed person.
BEGIN;
SET LOCAL lock_timeout='5s';
ALTER TABLE public.schedule_event_staff_assignments ADD COLUMN role_confirmed boolean NOT NULL DEFAULT true;
UPDATE public.schedule_event_staff_assignments a SET role_confirmed=false
FROM public.schedule_events e WHERE e.id=a.event_id
  AND nullif(e.gm_roles->>a.staff_name,'') IS NULL
  AND EXISTS(SELECT 1 FROM jsonb_object_keys(coalesce(e.gm_roles,'{}'::jsonb)) key
    WHERE NOT(key=ANY(coalesce(e.gms,ARRAY[]::text[]))));
