-- create_reservation_with_lock_v2 の CUSTOMER_ORG_MISMATCH チェックを修正
--
-- platform customer (organization_id = NULL) は全組織で予約可能なため
-- v_customer_org_id IS NOT NULL の場合のみ org 一致チェックを行う
...
CREATE OR REPLACE FUNCTION public.create_reservation_with_lock_v2(
  ...
  p_how_found text DEFAULT NULL::text,
  p_reservation_number text DEFAULT NULL::text,
  p_customer_coupon_id uuid DEFAULT NULL::uuid
) RETURNS uuid
