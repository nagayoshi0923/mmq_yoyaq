-- 買い切り作品フラグ: 作者への公演報告（ライセンス管理・送信タブ）から除外する
-- 破壊的変更なし。DEFAULT false。既存行は報告対象のまま。
--
-- ロールバック:
--   1) ビューを is_license_buyout なしの定義に戻す（本ファイル直前の live 定義）
--   2) ALTER TABLE public.organization_scenarios DROP COLUMN IF EXISTS is_license_buyout;

ALTER TABLE public.organization_scenarios
  ADD COLUMN IF NOT EXISTS is_license_buyout boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.organization_scenarios.is_license_buyout IS
  'true=買い切り作品。作者への公演報告（ライセンス管理の送信タブ）に出さない';

-- ビュー: 2026-08-21 staging live（pg_get_viewdef）に末尾1列追加のみ
