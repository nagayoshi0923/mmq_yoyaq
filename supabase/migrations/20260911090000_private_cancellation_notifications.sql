-- QW-20260911-002: 貸切取消と通知の記録を同一トランザクションにする。
-- 既存データへの通知の遡及送信はしない。既存の予約RPCは変更しない。
ALTER TABLE public.schedule_events ADD COLUMN IF NOT EXISTS gm_cancel_epoch uuid DEFAULT gen_random_uuid();
