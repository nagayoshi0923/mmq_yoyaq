-- organization_scenarios に貸切募集期間カラムを追加
...
COMMENT ON COLUMN public.organization_scenarios.booking_start_date IS '貸切募集開始日（NULLの場合は制限なし）';
COMMENT ON COLUMN public.organization_scenarios.booking_end_date IS '貸切募集終了日（NULLの場合は制限なし）';

-- ビューを更新して booking_start_date, booking_end_date を含める
...
COMMENT ON VIEW public.organization_scenarios_with_master IS 
  '組織シナリオとマスタを結合したビュー。貸切募集期間対応';

DO $$ 
BEGIN
  RAISE NOTICE '✅ 貸切募集期間カラム（booking_start_date, booking_end_date）を追加しました';
END $$;
