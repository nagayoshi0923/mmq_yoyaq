...
BEGIN
  ...
  LOOP
    ...
    IF v_event.one_seat_enabled AND NOT v_has_deadline AND v_min-v_current BETWEEN 1 AND v_event.max_missing_participants
      ...
      SELECT v_event.id,v_event.organization_id,r.id,COALESCE(NULLIF(btrim(r.customer_email),''),NULLIF(btrim(c.email),'')),
        jsonb_build_object('scenario',v_event.scenario,'date',v_event.date,'start_time',v_event.start_time,
          ...
          'was_confirmed',v_prior_confirmed,'site_url',v_event.customer_site_url,'missing_participants',v_min-v_current)
