-- 貸切リクエストの参加人数上限を10→50へ引き上げる(本番障害の緊急修正 2026-08-19)
-- 背景: クライアントは貸切送信時に人数として"シナリオの最大人数"(maxParticipants)を自動送信する
--   (usePrivateBookingSubmit.ts)。RPCの固定上限 IF p_participant_count > 10 のため、
--   最大11名以上のシナリオは貸切が必ず P0025(参加人数が上限を超えています)で弾かれていた。
-- 変更点は create_private_booking_request の上限チェック1行のみ(関数本体は 20260723210000 と同一)。
-- 恒久策(シナリオ定員と照合)は別途起票。
