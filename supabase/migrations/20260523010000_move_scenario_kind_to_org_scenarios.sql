-- scenario_kind を scenario_masters から organization_scenarios へ移動
--
-- 経緯: 20260523000000 で scenario_masters.scenario_kind を追加したが、
--   1) ScenarioEditDialogV2 (店舗用ダイアログ) は scenario_masters に書き込まない設計
--   2) 「オンライン販売アイテム」「出張公演限定」の判定は店舗ごとに行いたい
--   ため、organization_scenarios に持たせ直す。
--
-- 既存データは scenario_masters.scenario_kind が全て 'regular' なので
-- バックフィルは organization_scenarios のデフォルト ('regular') 適用で十分。

-- 1. organization_scenarios に scenario_kind を追加
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_schema = 'public'
      AND constraint_name = 'organization_scenarios_scenario_kind_check'
  ) THEN
    ALTER TABLE public.organization_scenarios
      ADD CONSTRAINT organization_scenarios_scenario_kind_check
      CHECK (scenario_kind IN ('regular', 'online_item', 'offsite_only'));
  END IF;
END $$;

COMMENT ON COLUMN public.organization_scenarios.scenario_kind IS
  'シナリオ種別: regular=通常 / online_item=オンライン販売アイテム(予約サイト非掲載) / offsite_only=出張公演限定(貸切不可)';

-- 2. ビュー先に DROP（scenario_masters.scenario_kind に依存しているため）
DROP VIEW IF EXISTS public.organization_scenarios_with_master;

-- 3. scenario_masters.scenario_kind を削除
ALTER TABLE public.scenario_masters
  DROP CONSTRAINT IF EXISTS scenario_masters_scenario_kind_check;
ALTER TABLE public.scenario_masters
  DROP COLUMN IF EXISTS scenario_kind;

-- 4. ビュー再構築 (organization_scenarios_with_master)
-- 変更点: sm.scenario_kind → os.scenario_kind
