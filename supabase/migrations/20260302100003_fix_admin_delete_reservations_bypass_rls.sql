-- 20260302100003: admin_delete_reservations_by_schedule_event_ids を修正
-- 問題: SECURITY DEFINER 関数内でも、依存テーブル（gm_availability_responses, 
--       coupon_usages等）を削除する際にRLSポリシーがチェックされ、
--       呼び出し元ユーザーの権限が不足している場合に409エラーが発生
--
-- 解決策: 関数内で SET LOCAL を使用してRLSを一時的にバイパス
--         これはトランザクション内のみ有効で、関数終了後は元に戻る
...
BEGIN
  ...
  -- ============================================================
  -- 依存テーブルの削除（RLSをバイパスするためにSECURITY DEFINERの
  -- 権限で直接削除を実行）
  -- 
  -- 注意: SECURITY DEFINER関数のオーナー（通常はpostgres）として
  -- 実行されるため、RLSは適用されない
  -- ============================================================
...
BEGIN
  ...
  RAISE NOTICE '   - is_staff_or_admin() チェックを追加';
  RAISE NOTICE '   - SECURITY DEFINER により依存テーブルのRLSをバイパス';
END $$;
