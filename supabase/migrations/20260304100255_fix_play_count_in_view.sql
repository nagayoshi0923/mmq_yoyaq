-- organization_scenarios_with_master ビューの play_count を修正
-- schedule_events から動的に計算するようにする
-- 作成日: 2026-03-04
...
SELECT
  ...
  -- play_count: schedule_events から動的に計算（今日までの公演、中止除外、出張除外）
  (
...
COMMENT ON VIEW public.organization_scenarios_with_master IS 
  '組織シナリオとマスタを結合したビュー。play_countはschedule_eventsから動的計算';
