-- 配役方法を organization_scenarios に移動（シナリオ単位で管理）
...
COMMENT ON COLUMN public.organization_scenarios.character_assignment_method IS '配役方法: survey=アンケートで希望を聞く, self=参加者同士で決める';

-- private_groups からは削除（前のマイグレーションで追加した分）
ALTER TABLE public.private_groups
DROP COLUMN IF EXISTS character_assignment_method;
...
SELECT
  ...
  COALESCE(os.character_assignment_method, 'survey') AS character_assignment_method
...
COMMENT ON VIEW public.organization_scenarios_with_master IS 
  '組織シナリオとマスタを結合したビュー。配役方法対応';

DO $$ 
BEGIN
  RAISE NOTICE '✅ 配役方法を organization_scenarios に移動しました';
END $$;
