-- 前日判定に dry_run を追加する（21:00 予告用）。
-- p_dry_run=TRUE なら schedule_events / performance_cancellation_logs を書かない。
-- 判定ロジックは 23:59 と同じ。破壊的変更なし（関数の差し替えのみ）。
-- ロールバック: DROP して 20260823020000 の DATE 1引数版に戻す。

DROP FUNCTION IF EXISTS public.check_performances_day_before(DATE);
...
'前日23:59に実行する公演中止判定（実予約＋未同期スタッフ・organization_scenariosの定員反映）。最低開催人数以上=開催確定（募集継続）/ 過半数以上=募集延長 / 過半数未満=中止。p_dry_run=TRUE なら書き込みしない（21:00予告）。p_target_dateを指定すると任意の日付を対象にできる。';

ALTER FUNCTION check_performances_day_before(DATE, BOOLEAN) SET timezone TO 'Asia/Tokyo';

GRANT EXECUTE ON FUNCTION public.check_performances_day_before(DATE, BOOLEAN) TO anon, authenticated, service_role;
