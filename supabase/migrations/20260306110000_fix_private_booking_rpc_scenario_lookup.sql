-- =============================================================================
-- マイグレーション: 貸切予約RPC関数のシナリオ検索を修正
-- =============================================================================
-- 
-- 作成日: 2026-03-06
-- 
-- 問題:
--   create_private_booking_request関数がscenariosテーブルを参照していたが、
--   フロントエンドはorganization_scenarios_with_masterビューのIDを渡している。
--   これによりSCENARIO_NOT_FOUNDエラーが発生していた。
-- 
-- 修正:
--   organization_scenarios_with_masterビューを参照するように変更
-- 
-- =============================================================================
...
BEGIN
  ...
  -- ==========================================================================
  -- シナリオ情報を取得（料金計算のため）
  -- organization_scenarios_with_master ビューを参照
  -- p_scenario_id は scenario_master_id（ビューのid列）
  -- ==========================================================================
  ...
  SELECT 
    ...
    organization_id,
    scenario_master_id  -- scenario_master_id を取得
...
'貸切予約リクエストを作成。organization_scenarios_with_masterビューを参照。料金はサーバー側で計算。';

-- 完了確認
DO $$ 
BEGIN
  RAISE NOTICE '✅ マイグレーション完了: 貸切予約RPC関数のシナリオ検索を修正';
END $$;
