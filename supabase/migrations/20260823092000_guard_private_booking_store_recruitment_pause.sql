-- 貸切申請・承認で店舗の貸切募集停止期間をセル停止と同じく拒否する
-- 正本: supabase/rpcs/create_private_booking_request.sql / approve_private_booking.sql
-- ロールバック: 直前の live 定義に戻す
...
GRANT EXECUTE ON FUNCTION public.approve_private_booking(UUID, DATE, TIME, TIME, UUID, UUID, JSONB, TEXT, TEXT, UUID) TO authenticated;
