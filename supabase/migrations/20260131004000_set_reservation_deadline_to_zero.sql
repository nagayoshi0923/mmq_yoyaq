...
-- 当日予約締切も「公演開始まで（0時間前）」に統一（店舗設定）
ALTER TABLE public.reservation_settings
  ALTER COLUMN same_day_booking_cutoff SET DEFAULT 0;

UPDATE public.reservation_settings
SET same_day_booking_cutoff = 0
WHERE same_day_booking_cutoff IS DISTINCT FROM 0;

COMMENT ON COLUMN public.reservation_settings.same_day_booking_cutoff
  IS '当日予約締切（公演開始の何時間前）。0 の場合は公演開始まで予約可。';
