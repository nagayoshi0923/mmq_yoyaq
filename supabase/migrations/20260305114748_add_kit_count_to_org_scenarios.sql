-- organization_scenarios に kit_count カラムを追加
-- 作成日: 2026-03-05

-- 1. organization_scenarios テーブルに kit_count カラムを追加
ALTER TABLE public.organization_scenarios
  ADD COLUMN IF NOT EXISTS kit_count INTEGER DEFAULT 1;

COMMENT ON COLUMN public.organization_scenarios.kit_count IS 
  'キット数（組織ごとのキット管理用）';

-- 2. 既存データを scenarios テーブルから移行
UPDATE public.organization_scenarios os
SET kit_count = s.kit_count
FROM public.scenarios s
WHERE s.scenario_master_id = os.scenario_master_id
  AND s.organization_id = os.organization_id
  AND s.kit_count > 1;

-- 3. organization_scenarios_with_master ビューを更新して kit_count を反映
...
COMMENT ON VIEW public.organization_scenarios_with_master IS 
  '組織シナリオとマスタを結合したビュー。kit_countは組織ごとの設定を反映';
