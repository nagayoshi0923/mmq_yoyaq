...
-- pricing_settings（テーブルが存在する場合のみ）
DO $$
BEGIN
...
-- notification_settings（テーブルが存在する場合のみ）
DO $$
BEGIN
...
-- reservation_settings（テーブルが存在する場合のみ）
DO $$
BEGIN
...
-- cancellation_settings（テーブルが存在する場合のみ）
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cancellation_settings') THEN
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns 
...
-- business_hours_settings（テーブルが存在する場合のみ）
DO $$
BEGIN
...
-- sales_report_settings（テーブルが存在する場合のみ）
DO $$
BEGIN
...
-- performance_schedule_settings（テーブルが存在する場合のみ）
DO $$
BEGIN
...
-- data_management_settings（テーブルが存在する場合のみ）
DO $$
BEGIN
...
-- booking_notices（テーブルが存在する場合のみ）
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'booking_notices') THEN
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns 
...
-- pricing_settings RLSポリシー（テーブルが存在する場合のみ）
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'pricing_settings') THEN
...
-- notification_settings RLSポリシー（テーブルが存在する場合のみ）
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'notification_settings') THEN
