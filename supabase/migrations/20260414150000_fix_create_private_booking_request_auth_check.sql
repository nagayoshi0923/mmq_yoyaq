...
BEGIN
  ...
  END IF;
  -- p_customer_id が指定されている場合、その customers レコードが自分のものであることを確認
  IF p_customer_id IS NOT NULL THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.customers WHERE id = p_customer_id AND user_id = auth.uid()
    ) THEN
      RAISE EXCEPTION 'Unauthorized: customer does not belong to authenticated user' USING ERRCODE = 'P0401';
    END IF;
  END IF;
