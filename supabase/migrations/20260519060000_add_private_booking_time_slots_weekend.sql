-- 平日/土日祝 別の貸切受付時間枠サポートを追加
--
-- 変更内容:
--   1. organization_scenarios に private_booking_time_slots_weekend を追加
--      - private_booking_time_slots     = 平日用（既存流用）
--      - private_booking_time_slots_weekend = 土日・祝日用（新規）
--   2. organization_scenarios_with_master ビューに新カラムを追加
--   3. private_booking_blocked_slots は廃止（UIから削除、カラムは互換のため残す）

ALTER TABLE public.organization_scenarios
  ADD COLUMN IF NOT EXISTS private_booking_time_slots_weekend text[];
