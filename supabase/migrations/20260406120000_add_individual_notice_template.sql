-- organization_scenarios に個別お知らせ定型文カラムを追加
ALTER TABLE public.organization_scenarios
ADD COLUMN IF NOT EXISTS individual_notice_template TEXT;

COMMENT ON COLUMN public.organization_scenarios.individual_notice_template IS '個別お知らせ送信時に添付できる定型文テンプレート';

-- ビューを更新して individual_notice_template を含める
...
COMMENT ON VIEW public.organization_scenarios_with_master IS 
  '組織シナリオとマスタを結合したビュー。個別お知らせ定型文対応';

DO $$ 
BEGIN
  RAISE NOTICE '✅ 個別お知らせ定型文カラム（individual_notice_template）を追加しました';
END $$;
