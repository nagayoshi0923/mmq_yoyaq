-- 再欠員の通知・無料辞退を周回で分離。既存行は周回1として維持する。
ALTER TABLE public.performance_recruitment_deadlines ADD COLUMN cycle integer NOT NULL DEFAULT 1 CHECK (cycle > 0);
ALTER TABLE public.performance_recruitment_notices ADD COLUMN cycle integer NOT NULL DEFAULT 1 CHECK (cycle > 0);
ALTER TABLE public.performance_recruitment_notices DROP CONSTRAINT performance_recruitment_notic_schedule_event_id_reservation_key;
ALTER TABLE public.performance_recruitment_notices ADD CONSTRAINT performance_recruitment_notices_cycle_unique UNIQUE(schedule_event_id,reservation_id,kind,cycle);
