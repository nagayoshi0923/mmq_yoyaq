...
CREATE TABLE schedule_events (id uuid PRIMARY KEY, organization_id uuid, date date, start_time time, scenario text, category text DEFAULT 'open', is_cancelled boolean DEFAULT false, is_recruitment_extended boolean DEFAULT true, gm_roles jsonb DEFAULT '{}', max_participants integer DEFAULT 4, organization_scenario_id uuid, scenario_master_id uuid, scenario_id uuid, store_id uuid, gms text[] DEFAULT '{}', updated_at timestamptz);
CREATE TABLE reservations (schedule_event_id uuid, participant_count integer, status text DEFAULT 'confirmed', reservation_source text, participant_names text[]);
