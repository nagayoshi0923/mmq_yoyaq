...
type Settings = { enabled: boolean; source: 'common' | 'custom'; mode: RecruitmentTarget['mode']; value: number; deadline_minutes: number; max_missing?: number }
type History = { id: string; actor_name: string; before_settings: Settings; after_settings: Settings; created_at: string }
type Response = { setting: { recruitment_extension_enabled: boolean; recruitment_max_missing: number; recruitment_deadline_minutes: number; recruitment_target_source: Settings['source']; recruitment_target_mode: RecruitmentTarget['mode']; recruitment_target_value: number; updated_at: string } | null; common: RecruitmentTarget; min_required: number; history: History[]; can_edit: boolean }
const describe = (s: Settings) => s.enabled ? `${s.source === 'common' ? '共通設定' : `不足${recruitmentTargetLabel({ mode: s.mode ?? 'count', value: s.value ?? s.max_missing ?? 2 })}`}・開始${s.deadline_minutes}分前まで` : '延長しない'
...
export function RecruitmentSettingsSection({ masterId, eventId, readOnly = false, minimumPlayers }: { masterId?: string | null; eventId?: string; readOnly?: boolean; minimumPlayers?: number }) {
  ...
  const [value, setValue] = useState<Settings>({ enabled: true, source: 'common', ...DEFAULT_RECRUITMENT_TARGET, deadline_minutes: 90 })
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(result)
      if (result.setting) setValue({ enabled: result.setting.recruitment_extension_enabled, source: result.setting.recruitment_target_source, mode: result.setting.recruitment_target_mode, value: result.setting.recruitment_target_value, deadline_minutes: result.setting.recruitment_deadline_minutes })
  ...
  return <section className="scenario-edit-card">
    ...
    {loading ? <p className="scenario-edit-card__note" role="status">読み込み中…</p> : !data?.setting ? !error && <p className="scenario-edit-card__help">シナリオを保存すると設定できます。</p> : <>
      ...
      {value.enabled && <>
        ...
        <div className="scenario-edit-field"><Label className="scenario-edit-field__label" htmlFor="recruitment-minutes">追加募集の期限</Label><div className="scenario-edit-field__control flex items-center gap-2"><Input id="recruitment-minutes" className="h-7 text-xs w-24" type="number" min={1} max={239} value={Number.isFinite(value.deadline_minutes) ? value.deadline_minutes : ''} onChange={e => setValue({ ...value, deadline_minutes: e.target.valueAsNumber })} disabled={disabled} /><span className="scenario-edit-card__help">分前（初期値：90分）／シナリオごとに変更可</span></div></div>
      </>}
      ...
      {missing !== null && Number.isInteger(value.deadline_minutes) && value.deadline_minutes >= 1 && value.deadline_minutes <= 239 && <p className="scenario-edit-card__note">18:00開演の例：{value.enabled && missing > 0 ? `14:00に判断し、不足${missing}人以内なら${time}まで募集。期限まで最低開催人数に届かなければ中止します。` : '14:00の判断で最低開催人数未達なら中止します。'}</p>}
      <p className="scenario-edit-card__note">共通設定は「設定 → 運用と案内 → 開催判断・追加募集」で変更できます。未案内の既存公演にも反映し、お客様へ案内済みの期限・不足人数の上限は維持します。</p>
