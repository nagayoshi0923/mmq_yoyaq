/**
 * 公演モーダル「日時・場所」セクション（PerformanceModal から子コンポーネント抽出・挙動不変）。
...
import { CATEGORY_TONE, timeOptions } from '../constants'

interface TimeSlotDefault { start_time: string; end_time: string; label: string }

interface DateTimeVenueSectionProps {
  formData: EventFormData
  setFormData: Dispatch<SetStateAction<EventFormData>>
  stores: Store[]
  timeSlot: 'morning' | 'afternoon' | 'evening'
  handleTimeSlotChange: (slot: 'morning' | 'afternoon' | 'evening') => void
  timeSlotDefaults: { morning: TimeSlotDefault; afternoon: TimeSlotDefault; evening: TimeSlotDefault }
  handleStartTimeChange: (startTime: string) => void
}

export function DateTimeVenueSection({
  formData,
  setFormData,
  stores,
  timeSlot,
