...
export function StaffCheckinBubble({ onStaffCheckin, checkinOnly = false }: { onStaffCheckin?: (staffId: string) => void; checkinOnly?: boolean }) {
  ...
  const shouldShowBubble = showActionBubble || showCheckinStatus
  return (
    <>
      {shouldShowBubble && <aside
        className={showActionBubble
          ? 'fixed bottom-5 right-5 z-50 w-[320px] rounded-2xl border border-indigo-200 bg-white p-5 shadow-xl'
          : 'fixed bottom-5 right-5 z-50 flex items-center gap-3 rounded-xl border border-emerald-200 bg-white px-4 py-3 shadow-lg'}
        aria-label={showActionBubble ? '出勤打刻' : '出勤打刻済み'}
      >
        {showActionBubble ? <>
