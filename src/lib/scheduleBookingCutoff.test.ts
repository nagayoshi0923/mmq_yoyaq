...
async function request(minutes: unknown) {
 const res = { setHeader: vi.fn(), status: vi.fn(), json: vi.fn(), end: vi.fn() }
 res.status.mockReturnValue(res)
 await handler({ method: 'PATCH', headers: {}, query: { action: 'booking-cutoff', id: 'event' }, body: { minutes, expected_updated_at: '2026-09-08T00:00:00+09:00', organization_id: 'untrusted' } } as never, res as never)
 return res
}
describe('公演別予約締切API', () => {
