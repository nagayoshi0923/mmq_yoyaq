...
export type ScenarioTimingFromDb = {
  ...
  /** シナリオが受け付ける貸切時間帯・平日（未設定＝全枠許可） */
  private_booking_time_slots?: string[] | null
  /** シナリオが受け付ける貸切時間帯・土日祝（未設定＝平日設定を流用） */
...
export async function fetchScenarioTimingFromDb(
  ...
  if (organizationId) {
    const { data: viewRow } = await supabase
      ...
      .select('duration, weekend_duration, extra_preparation_time, private_booking_time_slots, private_booking_time_slots_weekend')
