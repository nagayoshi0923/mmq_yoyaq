...
const chapters: Chapter[] = [
  {
    id: 'program',
    no: 1,
    title: 'プログラムとは「入力 → 変換 → 出力」',
    subtitle: 'まずは1つの関数を読めるようになる',
    sections: [
      {
  ...
  const t = getJstParts(value)        // ① 入力を年・月・日・曜日に分解
  if (!t) return ''                   // ② 変な入力なら空文字を返す（防御）
  const base = \`\${t.y}年\${t.mo}月\${t.d}日\`   // ③ 部品を組み立てる
  return withWeekday ? \`\${base}(\${t.weekday})\` : base  // ④ 曜日付きか選んで出力
      ...
      },
      {
        ...
        code: {
          ...
          body: `const result = await handleApprove(...)   // 承認処理を実行して結果を待つ
if (result?.success) {
  ...
  showToast.error(getSafeErrorMessage(result.error, '処理に失敗しました'))
        ...
        story: '「確定したか分からない」と報告してくれたバグの正体は、この場合分けに『成功のとき』の枝が無かったこと。失敗時しか書かれていなかったので、成功すると何も起きませんでした。',
      },
    ],
  },
  {
