...
// → 'えいきちさん、7名で受付ました'`,
      ...
      {
        heading: 'よくある疑問: const は変数？定数？',
        text: '答え：「const は再代入できない変数」です。変数の宣言には let（後から入れ替え可）と const（入れ替え不可 = constant が名前の由来）の2種類があり、現代のJSでは const が基本形。日本語の「定数」(π=3.14のような不変の決まりごと)とは少しズレていて、定数として使うものは大文字で書く慣習です（例: このシステムの QUEENS_WALTZ_ORG_ID）。会話では「constで宣言した変数」と言えば正確です。',
        code: {
          caption: 'let と const の違い ＋ constの落とし穴',
          body: `let count = 0
count = 5                  // ✅ let は入れ替えOK

const name = 'えいきち'
name = 'sososo'            // ❌ エラー！ const は入れ替え不可

// ⚠️ 落とし穴: const は「箱の取り替え禁止」であって「中身」は触れる
const reservation = { name: 'えいきち', count: 7 }
reservation.count = 8      // ✅ OK（中身の変更）
reservation = 別の予約      // ❌ ダメ（箱ごと交換）`,
        },
      },
      {
