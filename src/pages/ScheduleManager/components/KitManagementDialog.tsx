...
import type { KitTransferSuggestion, KitCondition } from '@/types'
...
import type { DraggedKit, ContextMenuState, DeliveryConfirmState, KitManagementDialogProps } from './kitManagement/types'
...
import { useKitManagementHandlers } from './kitManagement/useKitManagementHandlers'
...
export function KitManagementDialog({ isOpen, onClose }: KitManagementDialogProps) {
  ...
  // 設置確認ダイアログ
  const [deliveryConfirm, setDeliveryConfirm] = useState<DeliveryConfirmState | null>(null)
  ...
  // イベントハンドラ群（Phase 5-1 第6c歩で useKitManagementHandlers へ分離）
  const {
    handleTogglePickup,
