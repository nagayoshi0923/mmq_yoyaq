...
async function fetchScenarioDetail(scenarioId: string, organizationSlug?: string) {
  ...
  const scenarioEvents = eventsData
    .filter((event: any) => {
      ...
      // 貸切公演は表示しない
      if (event.is_private_booking === true) return false
      // 予約受付オフのイベントは表示しない（nullはデフォルトtrue扱い）
  ...
  const scenario: ScenarioDetail = {
    ...
    private_booking_slot_start_times: (scenarioData as { private_booking_slot_start_times?: unknown }).private_booking_slot_start_times ?? null,
