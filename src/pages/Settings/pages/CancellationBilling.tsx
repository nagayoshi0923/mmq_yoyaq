...
export function CancellationBilling() {
  ...
  const [contactChannel, setContactChannel] = useState<'company_email' | 'manual'>('company_email')
  ...
  async function intake(apply: boolean) {
    ...
    const result = await apiClient.post<{ assessment: FeeAssessment; preview: string }>('/api/cancellation-billing?action=intake', {
      ...
      contact: old?.data.contact?.channel === 'mmq' ? old.data.contact : { channel: contactChannel, ...(mailMessageId ? { messageId: mailMessageId } : {}), ...(mailThreadId ? { threadId: mailThreadId } : {}) },
  ...
  return <div className="space-y-6 max-w-4xl mx-auto pb-12">
    ...
    <Card><CardHeader><CardTitle>振込先と連絡設定（組織共通）</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      <label className="flex gap-2"><input type="checkbox" checked={settings.notificationsEnabled} onChange={e => setSettings({ ...settings, notificationsEnabled: e.target.checked })} />MMQで受けたキャンセルの料金案内・入金確認・口座変更の連絡を有効にする</label>
    ...
    <Card><CardHeader><CardTitle>キャンセル料の受付</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div><Label htmlFor="billing-contact">受付経路</Label><select id="billing-contact" className="border p-2 w-full" value={contactChannel} onChange={e => setContactChannel(e.target.value as 'company_email' | 'manual')}><option value="company_email">会社メール（元のメールへ返信）</option><option value="manual">電話・その他（運営が連絡）</option></select></div>
      ...
      <p>会社メールで受けた連絡には、会社のメールアドレスから元のメールへ返信します。この画面から別のMMQメールは送りません。募集延長・追加募集などの公演通知はMMQから案内します。</p>
      {contactChannel === 'company_email' && <details><summary>元メールの連携情報</summary><div className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="billing-mail-message">メール連携のメッセージID</Label><Input id="billing-mail-message" value={mailMessageId} onChange={e => setMailMessageId(e.target.value)} /></div><div><Label htmlFor="billing-mail-thread">メール連携のスレッドID</Label><Input id="billing-mail-thread" value={mailThreadId} onChange={e => setMailThreadId(e.target.value)} /></div></div></details>}
    ...
    <Card><CardHeader><CardTitle>入金照合と連絡</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant="outline" disabled={busy} onClick={() => void run(async () => {
          const result = await apiClient.post<{ notices: { text: string; deliveryChannel: string }[]; unknown: unknown[] }>('/api/cancellation-billing?action=notices', { apply: false })
          setPreview(result.notices.map(n => `${n.deliveryChannel === 'company_email' ? '【会社メールから元のメールへ返信・MMQ送信対象外】' : n.deliveryChannel === 'manual' ? '【運営から連絡・MMQ送信対象外】' : '【MMQ通知】'}\n${n.text}`).join('\n\n────────\n\n') || '新しい連絡はありません。')
